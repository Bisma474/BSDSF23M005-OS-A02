/* src/ls-v1.0.0.c  — Feature-3 (column "down then across") */
/*
 * lsv2.0.0 base updated for Feature-3
 * - Reads directory into memory
 * - Displays entries in columns "down then across" (auto width)
 * - Supports -l flag (existing long listing preserved)
 *
 * Keep this file name as src/ls-v1.0.0.c
 */
...
char **read_dir_all(const char *path, int *out_count, int *out_maxlen, int show_hidden) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_name[0] == '.') continue; /* skip hidden by default */
        if (n >= cap) {
            cap *= 2;
...
/* ----- existing code (mostly preserved) ----- */
...
/* list_simple now uses read_dir_all + display_down_across */
void list_simple(const char *path) {
    int n = 0, maxlen = 0;
    char **names = read_dir_all(path, &n, &maxlen, 0);
    if (!names) return;
    /* no sorting in Feature-3 */
    display_down_across(names, n, maxlen);
    free_names(names, n);
}

/* preserved long listing */
void list_long(const char *path) {
    DIR *dir = opendir(path);
    if (!dir) {
        perror("opendir");
